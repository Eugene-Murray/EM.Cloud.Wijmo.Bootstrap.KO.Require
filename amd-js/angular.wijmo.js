        ...
        */
        ...
        var Markup = (function () {
            ...
            */
            ...
            Markup.prototype.moveContents = /** move elements and id/class/style attributes from one element to another */
            function (from, to) {
            ...
            Markup.prototype.extractSubelements = /** If element matches the innerMarkupSelector, extract the contents and $compile */
            function (element) {
            ...
            Markup.prototype.parseInnerMarkup = /** Extract non-option inner markup from the element, prepare and put to this.subelements */
            function (element) {
                var _this = this;
            ...
            Markup.prototype.apply = /** Render parsed subelements to the parentElement */
            ...
            Markup.prototype.getNameMap = /** Create a (lowerCaseName => name) hash for the properties of an object */
            function (obj) {
                var map = {
                }, key;
                for(key in obj) {
            ...
            Markup.prototype.parseOptions = /** Extract options and put to this.options */
            function ($node, typeDef, events) {
                $.extend(true, this.options, this.parse($node, typeDef, events, ""));
            };
            Markup.prototype.parse = /** Parse options from a DOM tree according to widget metadata */
            function ($node, typeDef, events, path) {
                var _this = this;
                ...
                var readNode = function (node) {
                    ...
                    if(_this.isSpecialAttribute(name) && node.nodeType === Node.ATTRIBUTE_NODE) {
                        return;
                    }
                ...
                var node = $node[0], text = // read node text
                node.nodeType === Node.TEXT_NODE ? (node).wholeText : (node).value, isArray = typeDef && typeDef.type === "array", properties = typeDef && typeDef.properties, map = // we need this lowercase name map because HTML IS NOT CASE-SENSITIVE! Chris said that.
            ...
            Markup.prototype.extractBindingsAndEvents = /** Given an options object and metadata, extract data-bindings and event handlers */
            function (root, typeDef, events, path) {
                var _this = this;
                if(!root) {
                    return;
                }
            ...
            Markup.prototype.parseOptionsAttribute = /** Read the 'options' attribute and parse its contents */
            function ($node, typeDef, events) {
        ...
        */
        ...
        (function (definitions) {
            ...
            var DirectiveBase = (function () {
                ...
                DirectiveBase.mergeMetadata = /** Merge metadata from different sources: the widget type, its base types and default values */
                function mergeMetadata(widgetName, options) {
                    var fromOptions = {
                ...
                DirectiveBase.prototype.transition = /** Creates a new object prototyped from the current one
                * @remarks
                ...
                */
                ...
                DirectiveBase.prototype._compile = /** Perform compilation. Overridden in base classes */
                function (tElem, tAttrs, $compile) {
                    return $.proxy(this.link, this);
                };
                DirectiveBase.prototype._initWidget = /** Initialize the 'widget' field */
                function () {
                    this.widget = this.element.data(this.widgetName) || this.element.data(this.fullWidgetName);
                };
                DirectiveBase.prototype.link = /** Transition to the linked state */
                ...
                DirectiveBase.prototype._link = /** Perform linkage. Overridden in base classes */
                function (attrs) {
                    var _this = this;
                ...
                DirectiveBase.prototype._createInstance = /** Create a widget. To be overridden in derived classes */
                function (attrs) {
                };
                DirectiveBase.prototype.getEventFullName = //#region Events
                function (name) {
            ...
            var ElementDirective = (function (_super) {
                ...
                ElementDirective.prototype.createMarkup = /** Creates Markup object. The method may be overridden in a derived class.*/
                function (elem, meta) {
                    return new Markup(elem[0], meta, this.innerMarkupSelector, this.services);
                };
                ElementDirective.prototype.parseMarkup = /** Parse an element tree to options, data-binding entries and inner markup subelements */
                function (elem) {
                ...
                ElementDirective.prototype.watchData = /** Listen to changes of the "data" option. */
                ...
                ElementDirective.prototype.watchBinding = /** Listen to changes of an option */
                function (binding, handler) {
                ...
                ElementDirective.prototype.wireData = /** Process data-bindings */
                function (creationOptions) {
                    var _this = this;
                    var dataScope = this.$scope.$parent, applyingOptions = /** A set of options being applied at the moment */
                    {
                    }, changeEvents = {
                    };
                ...
                ElementDirective.prototype.setOption = /** Set option value
            ...
            var GridMarkup = (function (_super) {
                ...
                GridMarkup.prototype.parseOptions = // override parseOptions to extract cell templates
                function ($node, typeDef, events) {
